...
export default {
  ...
  created () {
    // if(loginBol) {
    //   this.$router.push('/')  //如果是登录页就返回到首页
    // } 
    // 先判断购物车页面是否存在用户id的值,如果没有就表示未登录,让用户地址转向到登录页面去
    // else if (!this.userInfo.id && !loginBol) {
    //   this.$router.push('/login') // 字符串,这里的字符串是路径path匹配噢，不是router配置里的name
    // }
    if (!this.userInfo.id) {
      this.$router.push('/login') // 字符串,这里的字符串是路径path匹配噢，不是router配置里的name
    } else {
      //  已经登录或者注册过了, 再判断一下userinfo是否存在地址信息
