<template>
  <div class="site">
    ...
    <div class="main">
      <!-- 没有地址的时候 -->
      <div class="empty-notice" v-if="!(addresses.length>0)">
        <div class="notice-icon"></div>
        您还没地址哦~
      </div>
      <!-- 显示地址列表 -->
      <ul class="addr-list block" v-if="addresses.length>0">
        <li class="addr-item spline-bottom" v-for="(item,index) in addresses" :key="item.id" @click="changeSelectedSite(item)">
